...
def build_landing():
  ...
  <span class="ic">{cl_icon.get(cl,"🎓")}</span><span class="nm">{esc(cl)}</span><span class="ct"><b>{cl_counts[cl]}</b> 个专业</span>
    ...
    faqs=[
     ("新加坡有几所理工学院？分别是哪几所？","共 5 所公立理工学院：新加坡理工学院（SP，1954）、义安理工学院（NP，1963）、南洋理工学院（NYP，1992）、淡马锡理工学院（TP，1990）、共和理工学院（RP，2002）。全部由政府资助、颁发受 NUS/NTU/SIT 等公立大学认可的 Diploma 文凭，与私立大专、培训机构不同。"),
     ("理工学院（Poly）和初级学院（JC）怎么选？","JC 是 2 年纯学术、考 A-Level 冲公立大学的路线，适合成绩拔尖、目标 NUS/NTU 直入的学生；理工是 3 年专业 + 实习、拿 Diploma 的路线，更偏实践、就业与升学两条腿走路。想早点接触专业、动手能力强、或想要更稳的升学缓冲，选理工更合适。"),
     ("中国学生可以申请理工学院吗？需要什么条件？","可以。国际学生可凭 O-Level 或同等学历（初中/高中成绩 + AEIS 或英文测试）申请，主流通过 JAE 或各校国际学生通道。通常需要数学、英文等基础科目达标，部分专业（如护理、设计）有附加面试或作品集要求。"),
     ("ELR2B2 是什么？分数怎么算？","ELR2B2 是 O-Level 升理工学院（JAE）的核心分数：英文（EL）+ 2 门相关科目（R2）+ 2 门最佳科目（B2），共 5 科之和，<b>越低越好</b>，可减 CCA 加分。大部分专业要求净分 ≤26，护理类（ELR2B2-C）放宽到 ≤28。具体每个专业的录取分见<a href='/poly/courses/'>专业录取分数据库</a>。"),
     ("理工 Diploma 毕业能升大学吗？","能，而且是主流去向之一。Diploma 毕业凭 GPA 可申请 SIT、NUS、NTU、SMU、SUTD 及英国、澳洲等海外大学，部分对口专业可获学分减免，最快 2 年完成本科。理工不是「终点」，而是通往大学的另一条路。"),
     ("理工学院读几年？学费多少？","全日制 Diploma 一般 3 年（含实习）。国际学生申请 MOE 学费津贴后约每年 S$11,000 左右（需签毕业后在新加坡工作 3 年的服务协议）；不申请津贴则更高。另需预算生活费、住宿与保险，整体仍比英美澳本科性价比高。"),
     ("5 所理工怎么选？各自强在哪？","看专业方向：SP 海事 / 视光 / 航空全国唯一、工程老牌；NP 传媒 / 电影、护理、商科强；NYP 护理 / 健康科学最强、教学工厂产业实战；TP 设计 / 兽医技术（全国唯一）、临湖校园；RP 问题导向学习 PBL、体育 / 酒店 / 航空。同一专业不同理工分数差别大，建议用数据库横向对比。"),
     ("没有 O-Level 的国际学生怎么进理工？","可走两条路：① 参加 AEIS / 同等英文与数学测试，或先读 O-Level 预备 / Foundation 预科再考 O-Level；② 凭高中成绩 + 校方测试直接申请部分理工的国际学生通道。具体门槛各校各专业不同，建议先做学历评估。"),
     ("理工学院毕业好就业吗？","新加坡理工 Diploma 在本地就业市场认可度高，很多专业（护理、工程、IT、酒店等）毕业即可对口就业，实习也常转正。多数学生会「先升学、后就业」或「边工作边读 part-time 学位」，路径灵活。"),
     ("男生读理工需要服兵役（NS）吗？","新加坡籍及部分 PR 男生需服兵役，通常在 Diploma 毕业后入伍，再升学或就业；国际学生（如中国学生）<b>不需要</b>服兵役，不受 NS 影响。"),
    ]
...
<td><div class="pdb-bar"><i class="{c["diff"]}" style="width:{fill}%"></i></div></td>
    ...
    cfaqs=[
     ...
     ("录取分区间「5-12」具体是什么意思？","左边的 5 是该专业当届分数最好（最低分）被录取者的成绩，右边的 12 是分数最低（最高分）被录取者的成绩，也就是俗称的「截分线」。换句话说，去年净 ELR2B2 在 5 到 12 之间的学生都被录取了；你的分数越接近或低于 12，把握越大。"),
     ("分数是越低越好还是越高越好？","越低越好。ELR2B2 是 5 科扣分制的总和，满分情形分数最小。所以表格里数字小的专业（红色）最难进、竞争最激烈；数字大的（绿色）门槛相对友好。排序默认「难→易」，点表头可切换。"),
     ("我的 O-Level 分数能进哪些理工专业？","在上方「我的 ELR2B2」输入你的净分并勾选「只看我能进的」，表格会只留下你够得着的专业（你的分数 ≤ 该专业最低录取分）。参考：约 12 分对应 150 多个专业、18 分约 60 多个、24 分约 24 个。再叠加学校或方向筛选即可精准缩小范围。"),
     ("同一个专业在不同理工，分数为什么差很多？","因为报考热度和招生名额不同。例如「资讯科技 IT」在 SP、NP、NYP、TP、RP 都有，但各校的口碑、地理位置和名额不一样，截分可以相差十几分。所以选专业一定要横向对比 5 所——用「方向」筛选最直观。"),
     ("护理为什么 ELR2B2 可以到 28，比别的专业宽？","护理（Nursing）使用的是 ELR2B2-C 这个特殊课程组，上限放宽到 28（其他专业是 26）。这是因为护理重视相关科目与志向匹配、且人才需求大，并不代表它「容易」——热门年份实际截分仍可能不低。"),
     ("分数不太好（20 分以上）还能读理工吗？","能。表格里不少专业截分在 20-26 之间（绿色档），尤其是部分工程、体育、酒店、设计大类方向。勾选「只看我能进的」输入你的分数，就能看到所有还够得着的选择；分数高也别灰心，理工 Diploma 一样能升大学。"),
     ("CCA 加分能减多少分？","课外活动（CCA）等表现最多可为 ELR2B2 减 2 分（净分 = 原始分 − 加分）。别小看这 2 分——在热门专业的截分线上，2 分往往就是录取与否的差别。本表展示的是减完加分后的净 ELR2B2。"),
     ("这些录取分是哪一年的？多久更新一次？","本数据库为 %s 年 JAE（最近一届）各专业的净 ELR2B2 录取区间，来源 MOE（data.gov.sg）及各理工官方招生页。JAE 每年 1 月 O-Level 放榜后进行，我们会按年更新。历史分数仅供参考，下一届实际截分会浮动。"%YEAR),
     ("国际学生（中国学生）也按 ELR2B2 录取吗？","凭新加坡 O-Level 走 JAE 的国际学生，规则与本地生一致、同样看 ELR2B2。若用国际学历（如中国高中成绩）走各校国际学生通道，则按各校自定标准评估，不直接套 ELR2B2，但本表的分数仍是判断专业冷热与竞争度的好参考。"),
    ]
