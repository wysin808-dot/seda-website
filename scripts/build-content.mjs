...
function updateBaiduRobotsAllowlist() {
  // 自动维护 robots.txt 中 Baiduspider 的原创内容白名单：
  // 模板校页前缀整体屏蔽，但其下的文章与手工页（SEDA_CUSTOM_PAGE）必须放行。
  const file = path.join(root, 'robots.txt');
  if (!fs.existsSync(file)) return 0;
  let txt = read(file);
  const start = '# SEDA_BAIDU_ALLOW_START';
  const end = '# SEDA_BAIDU_ALLOW_END';
  if (!txt.includes(start) || !txt.includes(end)) return 0;
  const blockedPrefixes = ['/primary-schools/', '/secondary-schools/', '/jc/', '/poly/', '/international-school/', '/private-schools/'];
  const isUnderBlocked = (u) => blockedPrefixes.some(p => u.startsWith(p) && u.length > p.length + 1);
  const allows = new Set();
  for (const a of articles) {
    const u = a.url.startsWith('/') ? a.url : '/' + a.url;
    if (isUnderBlocked(u)) allows.add(u.endsWith('/') ? u : u + '/');
  }
  for (const f of walk(root).filter(isSitemapPage)) {
    let rel = '/' + path.relative(root, f).replaceAll(path.sep, '/');
    if (!rel.endsWith('/index.html')) continue;
    rel = rel.slice(0, -'index.html'.length);
    if (isUnderBlocked(rel) && read(f).includes('<!-- SEDA_CUSTOM_PAGE -->')) allows.add(rel);
  }
  const body = [...allows].sort().map(u => 'Allow: ' + u).join('\n');
  const block = start + '\n# (auto-generated by build-content.mjs: original-content pages under blocked template prefixes)\n' + body + '\n' + end;
...
const baiduAllowCount = updateBaiduRobotsAllowlist();
writeFeeds(articles);
updateLlms(articles);
